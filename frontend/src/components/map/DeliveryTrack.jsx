...
const socket = io("http://localhost:5000"); /* import.meta.env.VITE_API_URL */
...
const customerIcon = new L.icon({
  ...
  shadowUrl:
    "https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-shadow.png", // Blue
  iconSize: [40, 40],
  iconAnchor: [20, 40],
  popupAnchor: [0, -40],
});
    ...
    <div className="pt-10 flex justify-center mt-5">
      <div className="w-full max-w-4xl h-[400px] border rounded-lg overflow-hidden">
        ...
        >
          ...
          {deliveryPerson && (
            <Marker position={location} icon={deliverPersonIcon}>
              <Popup>
                ...
                <br />
                <small className="text-bold"> {deliveryPerson.etaText} </small>
                <br />
              </Popup>
            </Marker>
          )}
          ...
          {source && destination && (
            <Polyline positions={[source, destination]} color="blue"></Polyline>
          )}
        </MapContainer>
      </div>
    </div>
